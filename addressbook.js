...
lh+="<li>"+items[i].names;
lh+=" [ "+items[i].phone+" ] ";
lh+='<a href="#delete-id" class="deletebtn" contactid="'+items[i].id+'"> delete </a>'
lh+="</li>";
}
list.html(lh);
...
if(name=="" || phone==""){
$('#favourites').empty().html('the phone number or name field cannot be empty').show('slow');
}else if(isDuplicate(name,phone)){
$('#favourites').empty().html('the contact info you specified is already in the database').show('slow');
}else if(isNaN(new Number(phone))){
$('#favourites').empty().html('the phone field must contain valid numeric data').show('slow');
}else if(name.match(/d/)){
$('#favourites').empty().html('the name field must not contain numeric input').show('slow');
}else{
//call the ajax save function
$('#favourites').empty().html('saving....').show();
 $.ajax({
 url: 'addressbook.php',
...
//set the delete favourites
$('#favourites').empty().html('deleting...').show();
